...
const editDetailApp = (app: API.AppVO) => {
  if (app.id) {
    detailVisible.value = false
    router.push({ name: 'appEdit', params: { id: app.id }, query: { from: 'home' } })
  }
}
  ...
  <main class="home-page page-enter">
    <section class="hero">
      <div class="hero__brand-mark">
        <img src="/logo.png" alt="" /><span>AI 驱动的创作工作台</span>
      </div>
    ...
    <div class="gallery-shell">
      <section class="app-section">
        <div class="section-heading">
          <div>
            <span class="section-kicker">MY CREATIONS</span>
            <h2>我的应用</h2>
            <p>继续优化你创建的网站，或打开已经部署的作品。</p>
          </div>
          <a-input-search
        ...
        <a-spin v-else :spinning="myApps.loading">
          ...
          <div v-if="myApps.total > myApps.pageSize" class="pagination-row">
            <a-pagination
              ...
              v-model:page-size="myApps.pageSize"
              :total="myApps.total"
              :page-size-options="['4', '8', '12', '20']"
              show-size-changer
              @change="fetchMyApps"
              @show-size-change="fetchMyApps"
            />
          </div>
        </a-spin>
      </section>
      ...
      <section class="app-section app-section--featured">
        <div class="section-heading">
          <div>
            <span class="section-kicker">CURATED SHOWCASE</span>
            <h2>精选应用</h2>
            <p>看看大家用一句话创造出的优秀网站。</p>
          </div>
          <a-input-search
            v-model:value="featuredApps.appName"
            class="section-search"
            placeholder="搜索精选应用"
            allow-clear
        ...
        <a-spin :spinning="featuredApps.loading">
          ...
          <div v-if="featuredApps.total > featuredApps.pageSize" class="pagination-row">
            <a-pagination
              v-model:current="featuredApps.pageNum"
              v-model:page-size="featuredApps.pageSize"
              :total="featuredApps.total"
              :page-size-options="['4', '8', '12', '20']"
              show-size-changer
              @change="fetchFeaturedApps"
              @show-size-change="fetchFeaturedApps"
            />
          </div>
        </a-spin>
      </section>
    ...
    <AppDetailDrawer
      ...
      :app="detailApp"
      :editable="detailApp?.userId === loginUserStore.loginUser?.id"
      @edit="editDetailApp"
    />
  </main>
</template>
...
.hero::after {
  ...
  background: radial-gradient(ellipse, rgba(23, 105, 224, 0.25), transparent 67%);
  filter: blur(20px);
}
.hero__brand-mark {
  display: inline-flex;
...
:deep(.prompt-studio__input.ant-input) {
...
.prompt-studio__footer > span {
  color: #8a96a8;
  font-size: 12px;
}
.prompt-studio__footer :deep(.ant-btn-primary) {
  height: 46px;
  padding-inline: 22px;
  font-weight: 700;
...
.gallery-shell {
  ...
  margin: -50px auto 0;
  padding: 50px clamp(22px, 4vw, 52px) 58px;
...
.signin-empty {
  ...
  gap: 18px;
  padding: 28px;
  background: linear-gradient(120deg, #effaf6, #edf5ff);
...
@media (max-width: 700px) {
  ...
  .prompt-studio__footer :deep(.ant-btn) {
    width: 100%;
  }
  .gallery-shell {
    width: min(100% - 20px, var(--app-content-width));
